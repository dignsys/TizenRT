...
static int ssl_encrypt_buf( mbedtls_ssl_context *ssl )
{
        ...
        mode == MBEDTLS_MODE_CCM )
    {
        ...
        if ( !mbedtls_ssl_message_type_valid(ssl->out.msgtype) )
        {
            MBEDTLS_SSL_DEBUG_MSG( 1, ( "should never happen" ) );
            return( MBEDTLS_ERR_SSL_INTERNAL_ERROR );
        }
        ...
        add_data[8]  = (unsigned char)ssl->out.msgtype;
...
static int ssl_decrypt_buf( mbedtls_ssl_context *ssl )
{
        ...
        mode == MBEDTLS_MODE_CCM )
    {
        ...
        if ( !mbedtls_ssl_message_type_valid(ssl->in.msgtype) )
        {
            MBEDTLS_SSL_DEBUG_MSG( 1, ( "should never happen" ) );
            return( MBEDTLS_ERR_SSL_INTERNAL_ERROR );
        }
        ...
        add_data[8]  = (unsigned char)ssl->in.msgtype;
        mbedtls_ssl_write_version( ssl->major_ver, ssl->minor_ver,
                           ssl->conf->transport, add_data + 9 );
...
static int ssl_flight_append( mbedtls_ssl_context *ssl )
{
    ...
    msg->type = (unsigned char)ssl->out.msgtype;
    msg->next = NULL;
