...
if [ ${CONFIG_ENABLE_IOTIVITY} -eq 1 ]; then
	...
	CONFIG_ARCH_BOARD_STM32L4XR=`extract_flags "CONFIG_ARCH_BOARD_STM32L4XR"`
	if [ ${CONFIG_ARCH_BOARD_STM32L4XR} -eq 1 ]; then
		OPTIONS="TARGET_OS=tizenrt TARGET_ARCH=${IOTIVITY_TARGET_ARCH} TARGET_TRANSPORT=IP ROUTING=${IOTIVITY_ROUTING} DISABLE_TCP_SERVER=yes TC_PREFIX=arm-none-eabi- VERBOSE=no TIZENRT_OS_DIR=${TOPDIR} PLATFORM_TLS=${PLATFORM_TLS} ARCH_BOARD=stm32l4xr -j 8";
	else
