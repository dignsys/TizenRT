...
#define TX_BUFFER_MAX_LENGTH 10
static uint32_t __tx_buffer_index;
static wiced_buffer_t __tx_buffer[TX_BUFFER_MAX_LENGTH];
...
wwd_result_t wwd_buffer_init( /*@null@*/ void* native_arg )
{
	...
	for (i = 0; i < TX_BUFFER_MAX_LENGTH; ++i)
		__tx_buffer[i] = NULL;

	for (i = 0; i < TX_BUFFER_MAX_LENGTH; ++i) {
		__tx_buffer[i] = (wiced_buffer_t)kmm_malloc(sizeof(struct wiced_buffer));
		buff = __tx_buffer[i];
		if (buff == NULL)
			goto __err_mem_accloc;
		buff->flags = WICED_BUFFER_lwIP_NEED_FREE;
		buff->lwip_buf = pbuf_alloc(PBUF_RAW, WICED_LINK_MTU, PBUF_RAM);
		if (buff->lwip_buf == NULL)
			goto __err_mem_accloc;
		buff->org_paylaod = buff->lwip_buf->payload;
	}
...
wwd_result_t wwd_buffer_deinit(void)
{
	...
	for (i = 0; i < TX_BUFFER_MAX_LENGTH; ++i) {
		if (__tx_buffer[i]->lwip_buf != NULL)
			pbuf_free(__tx_buffer[i]->lwip_buf);
		if (__tx_buffer[i] != NULL)
			mem_free(__tx_buffer[i]);
		__tx_buffer[i] = NULL;
	}
...
wwd_result_t internal_host_buffer_get(wiced_buffer_t *buffer, wwd_buffer_dir_t direction, unsigned short size, unsigned long timeout_ms )
{
	...
	*buffer = NULL;
	...
	if (direction == WWD_NETWORK_TX) {
		if (++__tx_buffer_index == TX_BUFFER_MAX_LENGTH)
			__tx_buffer_index = 0;
		*buffer = __tx_buffer[__tx_buffer_index];
		(*buffer)->lwip_buf->len = size;
		(*buffer)->lwip_buf->tot_len = size;
		return WWD_SUCCESS;
	}

	*buffer = (wiced_buffer_t)kmm_malloc(sizeof(struct wiced_buffer));
	if (*buffer == NULL)
		return WWD_MALLOC_FAILURE;
