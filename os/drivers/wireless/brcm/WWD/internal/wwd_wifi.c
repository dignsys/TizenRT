...
#define DEFAULT_JOIN_ATTEMPT_TIMEOUT     (15000) /* Overall join attempt timeout in milliseconds. */
...
#define _ARASAN_BSS_INFO_VERSION_MASK 0x10000
    ...
    /* print out scan results info */
    {
        ...
        ether_ntoa(( const uint8_t * ) bss_info->BSSID.octet, ea_buf, sizeof( ea_buf ))));
    }
...
void wwd_log_event( const wwd_event_header_t* event_header, const uint8_t* event_data )
{
    ...
    /* DIGNSYS */
    WPRINT_WWD_INFO(("%lu: Event (interface, type, status, reason): %s, %s, %s, %s\n",
