...
#endif
#define MAX_SCAN_RESULT_BUFF     (60) /* Sure good enough? */
...
static uint32_t bss_current_index;
typedef struct {
	wl_bss_info_t info;
	uint8_t *ie_data;
} __bss_list_t;
...
static __bss_list_t *__bss_list[MAX_SCAN_RESULT_BUFF] = {NULL,};
...
static wiced_config_soft_ap_t gSoftap;
//static uint8_t ap_bssid[6]={0,};
static uint8_t *pap_bssid = NULL;
...
lldbg("***HBAHN[%d] : bss_index(%d)\n",__LINE__, bss_index);
	bss_info = (*result_ptr)->bi;
	__bss_list[bss_index] = (__bss_list_t *)kmm_zalloc(sizeof(__bss_list_t));
	__bss_list[bss_index]->ie_data = (uint8_t *)kmm_zalloc(bss_info->ie_length);
	memcpy(&__bss_list[bss_index]->info, bss_info, sizeof(wl_bss_info_t));
	memcpy(__bss_list[bss_index]->ie_data, ((uint8_t *)bss_info) + bss_info->ie_offset, bss_info->ie_length);

	__set_last_scan_result_index_increase();
	*result_ptr = __ap_scan_result_buff + __get_last_scan_result_index();
}

static wwd_result_t wpa_wiced_wifi_find_ap(const wiced_ssid_t* ssid, wiced_scan_result_t* ap_info, const uint16_t* optional_channel_list)
{
    wwd_result_t result;
 ...
 * uses global data.
 */
extern int stm32l4_brcm_wlan_netif_init(void);
void *cyw43438_init2(void *ctx, const char *ifname, void *global_priv)
{
	struct netif *dev;
...
void free_scan_buff(void)
{
	int i;
	for(i=0; i<MAX_SCAN_RESULT_BUFF; i++) {
		if(__bss_list[i] != NULL) {
			if(__bss_list[i]->ie_data != NULL)
				kmm_free(__bss_list[i]->ie_data);
			kmm_free(__bss_list[i]);
		}
	}
	if (__ap_scan_result_buff != NULL)
...
//	bi = NULL;
	...
	do {
		--num_scan_count;
		cyw43438_add_wpa_scan_entry(results, &__bss_list[num_scan_count]->info, __bss_list[num_scan_count]->ie_data);
...
static void* __associate_ok_event(void* data)
{
	...
	event.assoc_info.addr = pap_bssid;
//	usleep(100000);
	printf( "send EVENT_ASSOC: %02X:%02X\n", event.assoc_info.addr[4], event.assoc_info.addr[5]);
...
static int32_t __get_ies(const uint8_t *bssid, void* ies, uint32_t *ies_len)
{
	...
	index = count - 1;
	do {
		bi = &(__bss_list[index]->info);
		if (__is_same_data(bssid, bi->BSSID.octet, 6) == WICED_TRUE) {
			ies = __bss_list[index]->ie_data;
...
void save_softap_passphrase(uint8_t *passphrase)
{
	if(passphrase) {
		gSoftap.security_key_length = strlen((char*)passphrase);
		if(gSoftap.security_key_length >= SECURITY_KEY_SIZE)
			printf("security_key_length overflow!!\n");
		strcpy(gSoftap.security_key, (char*)passphrase);
		lldbg("key=%s\n", gSoftap.security_key);
	}
}
...
int cyw43438_associate(void *priv, struct wpa_driver_associate_params *request)
{
	...
	if(request->bssid)
		lldbg("bssid=%s\n", request->bssid);
	if(request->ssid_len && request->ssid)
		lldbg("ssid=%s, len=%d\n", request->ssid, request->ssid_len);
	if(request->wpa_proto)
		lldbg("wpa_proto=%d\n", request->wpa_proto);
	if(request->pairwise_suite)
		lldbg("pairwise_suite=%d\n", request->pairwise_suite);
	if(request->group_suite)
		lldbg("group_suite=%d\n", request->group_suite);
	if(request->key_mgmt_suite)
		lldbg("key_mgmt_suite=%d\n", request->key_mgmt_suite);
	if(request->auth_alg)
		lldbg("auth_alg=%d\n", request->auth_alg);
	if(request->freq.freq)
		lldbg("freq=%d, band=%d, channel=%d\n", request->freq.freq, request->freq.bandwidth, request->freq.channel);
	if(request->wpa_ie && request->wpa_ie_len)
		lldbg("wpa_ie=%s, len=%d\n", request->wpa_ie, request->wpa_ie_len);
	...
	if (request->mode == 2) {
		if(request->ssid) {
			gSoftap.SSID.length = request->ssid_len;
			memcpy(gSoftap.SSID.value, request->ssid, gSoftap.SSID.length);
			lldbg("ssid=%s\n", gSoftap.SSID.value);
		}

		if(request->key_mgmt_suite == WPA_DRIVER_CAPA_KEY_MGMT_WPA
			|| request->key_mgmt_suite == WPA_DRIVER_CAPA_KEY_MGMT_WPA_PSK)
			gSoftap.security = WICED_SECURITY_WPA_AES_PSK;
		else if(request->key_mgmt_suite == WPA_DRIVER_CAPA_KEY_MGMT_WPA2
			|| request->key_mgmt_suite == WPA_DRIVER_CAPA_KEY_MGMT_WPA2_PSK)
			gSoftap.security = WICED_SECURITY_WPA2_AES_PSK;
		else
			gSoftap.security = WICED_SECURITY_OPEN;
		lldbg("key_mgmt_suite(%d)=>security(%d)\n", request->key_mgmt_suite, gSoftap.security);

		gSoftap.channel = request->freq.channel;
		lldbg("channel=%d\n", gSoftap.channel);
		return 0;
	}
...
int cyw43438_set_ap(void *priv, struct wpa_driver_ap_params *params)
{
    ...
    wwd_result_t result;
	struct netif *dev;
	lldbg("cyw43438_set_ap\n");

	if(params->ssid == NULL)
		goto ap_reject;
	if(strncmp(params->ssid, gSoftap.SSID.value, gSoftap.SSID.length) != 0) {
		lldbg("invalid ssid=%s\n", params->ssid);
		goto ap_reject;
	}

	lldbg("ssid=%s,secu=%x, key=%s, len=%d, channel=%d\n", gSoftap.SSID.value, gSoftap.security, gSoftap.security_key, gSoftap.security_key_length, gSoftap.channel);

	pthread_mutex_lock(&__wpa_interface_mutex);
	dev = get_cyw43438_wlan_netif();
	dev->state = WWD_AP_INTERFACE; 	/* change state to AP interface */
	result = (wwd_result_t) wwd_wifi_start_ap( &gSoftap.SSID, gSoftap.security, (uint8_t*) gSoftap.security_key, gSoftap.security_key_length, gSoftap.channel);
	pthread_mutex_unlock(&__wpa_interface_mutex);
	if ( result != WWD_SUCCESS )
	{
		printf( "Error: wwd_wifi_start_ap failed\n" );
		goto ap_reject;
	}

	if((pap_bssid= cyw43438_get_mac_addr(priv)) != NULL) {
		printf( "wwd_wifi_start_ap: %02X:%02X\n", pap_bssid[4], pap_bssid[5]);
//		pap_bssid = ap_bssid;
	}
	else {
		printf( "Error: get bssid\n" );
//		pap_bssid = NULL;
	}
	__send_associate_ok_event_to_wpa_supplicant((void *)((CYW43438_DRV_T *)priv)->ctx);
...
int cyw43438_stop_ap(void *priv)
{
	...
	if ( result != WWD_SUCCESS )
	{
		printf( "Error: wwd_wifi_stop_ap failed\n" );
		return -1;
	}
	...
	dev->state = WWD_STA_INTERFACE; 	/* change state to STATION interface */
...
//lldbg("*HBAHN[%d] : addr("CYW_MAC_FORMAT"), key_len(%d), key(%s) \n",__LINE__, CYW_MAC_STR(addr), key_len, key);
#if 0
	if(passphrase) {
		gSoftap.security_key_length = strlen((char*)passphrase);
		if(gSoftap.security_key_length >= SECURITY_KEY_SIZE)
